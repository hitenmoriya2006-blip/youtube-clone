...
const ShortsShelf = ({ shorts }) => {
  return (
    <div className="col-span-full py-4  border-y border-white/10 my-2">
      ...
      <div className="flex gap-2 overflow-x-auto  pb-4 snap-x snap-mandatory hide-scrollbar">
        {shorts.map((short) => (
          <div key={short.id} className="min-w-[200px] w-[200px] hover:bg-surface-container-highest shrink-0 snap-start cursor-pointer rounded-xl p-2.5 transition-transform duration-300 hover:bg-surface-container-highest group">
            <div className="relative w-full aspect-[9/16] rounded-xl overflow-hidden bg-surface-container-highest">
              <img alt={short.title} className="w-full h-full object-cover " src={short.thumbnail} />
