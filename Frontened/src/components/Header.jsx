...
const Header = ({ onMenuClick }) => {
  ...
  const searchQuery = (data) => {
      if(data.search.trim() === '') return
       ...
       navigate(`/result?query=${encodeURIComponent(data.search)}`)
  }
        ...
        className="flex w-full">
          <div className="flex w-full">
            <div className="flex flex-1 items-center bg-[#121212] border border-surface-variant rounded-l-full px-4 h-10 relative focus-within:border-[#1c62b9]">
              <input className="w-full bg-[#121212] border-none outline-none text-on-surface text-[16px] font-body-md placeholder:text-on-surface-variant" placeholder="Search" type="text" 
              autoComplete='off'
              {...register('search')}
              />
